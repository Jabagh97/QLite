...
{
    public class ProtocolHDot : IProtocolHDot
    {
        ...
        public ProtocolHDot()
        {
            ...
            //Font = new ProtocolHFont(DisplayFontStyle.Bold, DisplayArrowStyle.Thin);
        ...
        public byte[] ProduceSettingsMessage(byte displayNo, int flashCountVal, int dimmingTimeVal)
        {
        ...
        public byte[] ProduceMarquee(byte displayNo, string messageText, int messageIdleTime)
        {
        ...
        public byte[] ProduceMarqueeBitmap(byte displayNo, string messageText, VCTerminalSettings settings, bool rightToLeftDirection = false)
        {
            var buffer = new List<byte>();
            byte direction = rightToLeftDirection ? MARQUE_DIRECTION_RIGHT_TO_LEFT : MARQUE_DIRECTION_LEFT_TO_RIGHT;
            byte[] idleTimeBeforeScrollingDigits = GetDigits(settings.MessageIdleTime ?? 0, 0, 999);
            byte[] messageBuffer = TextToBitmap.MessageBitmap(messageText, rightToLeftDirection, settings);

            buffer.Add(displayNo);
            buffer.Add(direction);
            buffer.Add(idleTimeBeforeScrollingDigits[0]);
            buffer.Add(idleTimeBeforeScrollingDigits[1]);
            buffer.Add(idleTimeBeforeScrollingDigits[2]);
        ...
        public byte[] ProduceDisplayNumberMessage(byte displayNo, int rowId, int dotHeight, int dotWidth)
        {
        ...
        public byte[] ProduceArabicDelayTime(byte displayNo, int delayTime)
        {
        ...
        private byte[] GetDigits(string ticketno)
            => _defaultProtocolEncoding
                .GetBytes(ticketno.PadLeft(3, '0'))
                .Take(3)
                .ToArray();
    ...
    public interface IProtocolHDot
    {
        /// <summary>
        ...
        byte[] ProduceMarquee(byte displayNo, string messageText, int messageIdleTime);
        byte[] ProduceMarqueeBitmap(byte displayNo, string messageText, VCTerminalSettings settings, bool rightToLeftDirection = false);
        byte[] ProduceSettingsMessage(byte displayNo, int flashCountVal, int dimmingTimeVal);
        byte[] ProduceDisplayNumberMessage(byte displayNo, int rowId, int dotHeight, int dotWidth);
        byte[] ProduceArabicDelayTime(byte displayNo, int delayTime);
