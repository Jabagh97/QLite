...
namespace QLite.KioskLibrary.Display
{
    public class EmseDisplayHDot : IDisplay
    {
        ...
        public void Initialize(object settings)
        {
            ...
            Log.Debug($"{Name} initializing:" + JsonConvert.SerializeObject(Settings));
        ...
        {
            Log.Error($"DotDisplay qnumDataObject" + qnumDataObject?.TicketNo);
            ...
            if (queNum == null)
            {
                Log.Error($"Display not fount in settings {dispNo}");
            }
            DisplayType = DisplayFunction.CounterDisplay;
        ...
        private bool Display(int displayno, int bankoNo, string ticketno)
        {
            ...
            Log.Debug("display writing");
            var res = Device.Write(DisplayPackage.ToArray());
            Log.Debug("display writing done");
            return res;
        }
